...
public class CookerPanelFragment90 extends CookerPanelFragment implements CircleProgress.OnCircleProgressListener, TrianProgressView.OnTrianProgressListener {
    ...
    @BindView(R.id.cooker_view_middle)
    CircularCookerView cookerViewMiddle;
    ...
    private static final int everyForFanScreen = 0;
    ...
    protected int initLayout() {
        return R.layout.fragment_cooker_panel_fragment90;
    }
    ...
    protected void handleCookerErrors(int errorMessage, int cookerBits) {
        ...
        if (AnalysisSerialData.checkCookerC(cookerBits)) {
            cookerViewMiddle.showError(errorMessage);
        }

        if (rightCookersUnited()) {
            if (AnalysisSerialData.checkCookerE(cookerBits)
                    || AnalysisSerialData.checkCookerF(cookerBits)) {
                if (errorMessage > 0) {
            ...
            if (AnalysisSerialData.checkCookerF(cookerBits)) {
    ...
    protected void initialize() {
        ...
        // tvValueHint.setTypeface(Typeface.DEFAULT_BOLD);
        ...
        /*中间的圈圈*/
        cookerViewMiddle.setOnCircularCookerViewListener(new CircularCookerView.OnCircularCookerViewListener() {
            @Override
            public void onCookerPowerOn() {
                doPowerOnCooker(COOKER_ID_Middle);
            }

            @Override
            public void onCookerPowerOff() {
                doPowerOffCooker(COOKER_ID_Middle, DoAllStop);
            }

            @Override
            public void onCookerSelected(int value) {
                doSelectCooker(COOKER_ID_Middle, value);
            }

            @Override
            public void onUpdateCountDownGearB(String message) {
                updateCountDownGearBMessage(COOKER_ID_Middle, message);
            }
            ...
            public void onTimerIsUp(int v) {
                if (v == CookerView.set_to_stop_timeUp) {
                    doPowerOffCooker(COOKER_ID_Middle, DoTimerStop);
                }
                TFTCookerApplication.getInstance().updateLatestTouchTime();
                mSoundManager.playAlarm();
                handler.sendEmptyMessageDelayed(HANDLER_TIMER_IS_UP, DURATION_TO_SHUT_DOWN_AUDIO);
    ...
    private void setGearAfterBooster() {
    ...
    public int getCookersBits() {
        return AnalysisSerialData.COOKER_A_BIT
                ...
                | AnalysisSerialData.COOKER_E_BIT
                | AnalysisSerialData.COOKER_F_BIT;
    }
    ...
    private void doSyncGearValue(int value) {
        ...
        if (cookerViewMiddle_boost_flag && cookerViewMiddle.getTimeForGearB().equals(TIMER_ZERO_STRING)) {
            cookerViewMiddle.SetBoostWorkingStatus(false);
            cookerViewMiddle_boost_flag = false;
            if (workMode == WORK_MODE_HOB && currentCooker == COOKER_ID_Middle) {
                updateCircleProgressValue(value);
            }
        }
        ...
        if (boost_flag_CookerView_Up_Left_Down_Left_SameTime && cookerViewUpLeft.getTimeForGearB().equals(TIMER_ZERO_STRING)) {
            ...
            if (workMode == WORK_MODE_HOB && currentCooker == COOKER_ID_Up_Left_And_Down_Letf_SameTime) {
                updateCircleProgressValue(value);
            }
        }
        ...
        if (boost_flag_CookerView_Up_Right_Down_Right_SameTime && cookerViewUpRight.getTimeForGearB().equals(TIMER_ZERO_STRING)) {
            ...
            if (workMode == WORK_MODE_HOB && currentCooker == COOKER_ID_Up_Right_And_Down_Right_SameTime) {
                updateCircleProgressValue(value);
            }
        }
        ...
        if (boost_flag_CookerView_Up_Left__Middle && cookerViewMiddle.getTimeForGearB().equals(TIMER_ZERO_STRING)) {
            cookerViewUpLeft.SetBoostWorkingStatus(false);
            cookerViewMiddle.SetBoostWorkingStatus(false);
            boost_flag_CookerView_Up_Left__Middle = false;// 左上、中间
            if (workMode == WORK_MODE_HOB && currentCooker == COOKER_ID_Up_Left__Middle) {// 左上、中间 同时点击
                updateCircleProgressValue(value);
            }
        }

        if (boost_flag_CookerView_Up_Left__Middle_Slip && cookerViewMiddle.getTimeForGearB().equals(TIMER_ZERO_STRING)) {
            cookerViewUpLeft.SetBoostWorkingStatus(false);
            cookerViewMiddle.SetBoostWorkingStatus(false);
            boost_flag_CookerView_Up_Left__Middle_Slip = false;// 左上、中间  滑动
            if (workMode == WORK_MODE_HOB && currentCooker == COOKER_ID_Up_Left__Middle_Slip) {// 左上、中间 滑动
                updateCircleProgressValue(value);
            }
        }


        if (boost_flag_CookerView_Down_Left_Middle && cookerViewMiddle.getTimeForGearB().equals(TIMER_ZERO_STRING)) {
            cookerViewDownLeft.SetBoostWorkingStatus(false);
            cookerViewMiddle.SetBoostWorkingStatus(false);
            boost_flag_CookerView_Down_Left_Middle = false;// 左下、中间
            if (workMode == WORK_MODE_HOB && currentCooker == COOKER_ID_Down_Left_Middle) {// 左下、中间 同时点击
                updateCircleProgressValue(value);
            }
        }

        if (boost_flag_CookerView_Down_Left_Middle_Slip && cookerViewMiddle.getTimeForGearB().equals(TIMER_ZERO_STRING)) {
            cookerViewDownLeft.SetBoostWorkingStatus(false);
            cookerViewMiddle.SetBoostWorkingStatus(false);
            boost_flag_CookerView_Down_Left_Middle_Slip = false;// 左下、中间  滑动
            if (workMode == WORK_MODE_HOB && currentCooker == COOKER_ID_Down_Left_Middle_Slip) {// 左下、中间 滑动
                updateCircleProgressValue(value);
            }
        }

        if (boost_flag_CookerView_Up_Right_Middle && cookerViewMiddle.getTimeForGearB().equals(TIMER_ZERO_STRING)) {
            cookerViewUpRight.SetBoostWorkingStatus(false);
            cookerViewMiddle.SetBoostWorkingStatus(false);
            boost_flag_CookerView_Up_Right_Middle = false;// 右上、中间
            if (workMode == WORK_MODE_HOB && currentCooker == COOKER_ID_Up_Right_Middle) {// 右上、中间 同时点击
                updateCircleProgressValue(value);
            }
        }

        if (boost_flag_CookerView_Up_Right_Middle_Slip && cookerViewMiddle.getTimeForGearB().equals(TIMER_ZERO_STRING)) {
            cookerViewUpRight.SetBoostWorkingStatus(false);
            cookerViewMiddle.SetBoostWorkingStatus(false);
            boost_flag_CookerView_Up_Right_Middle_Slip = false;// 右上、中间  滑动
            if (workMode == WORK_MODE_HOB && currentCooker == COOKER_ID_Up_Right_Middle_Slip) {// 右上、中间  滑动
                updateCircleProgressValue(value);
            }
        }

        if (boost_flag_CookerView_Down_Right_Middle && cookerViewMiddle.getTimeForGearB().equals(TIMER_ZERO_STRING)) {
            cookerViewDownRight.SetBoostWorkingStatus(false);
            cookerViewMiddle.SetBoostWorkingStatus(false);
            boost_flag_CookerView_Down_Right_Middle = false;// 右下、中间
            if (workMode == WORK_MODE_HOB && currentCooker == COOKER_ID_Down_Right_Middle) {// 右下、中间 同时点击
                updateCircleProgressValue(value);
            }
        }

        if (boost_flag_CookerView_Down_Right_Middle_Slip && cookerViewMiddle.getTimeForGearB().equals(TIMER_ZERO_STRING)) {
            cookerViewUpRight.SetBoostWorkingStatus(false);
            cookerViewMiddle.SetBoostWorkingStatus(false);
            boost_flag_CookerView_Down_Right_Middle_Slip = false;//  右下、中间  滑动
            if (workMode == WORK_MODE_HOB && currentCooker == COOKER_ID_Down_Right_Middle_Slip) {// 右下、中间  滑动
                updateCircleProgressValue(value);
            }
        }
        ...
        if (boost_flag_CookerView_Up_Left_Down_Left_Up_Right && cookerViewUpLeft.getTimeForGearB().equals(TIMER_ZERO_STRING)) {
            ...
            if (workMode == WORK_MODE_HOB && currentCooker == COOKER_ID_Up_Right_Up_Left_Down_Left) {// 右上、左上、左下
                updateCircleProgressValue(value);
            }
        }
        ...
        if (boost_flag_CookerView_Up_Right_Down_Right_Up_Left && cookerViewUpLeft.getTimeForGearB().equals(TIMER_ZERO_STRING)) {
            ...
            if (workMode == WORK_MODE_HOB && currentCooker == COOKER_ID_Up_Left_Up_Right_Down_Right) {// 左上、右上、右下
                updateCircleProgressValue(value);
            }
        }
        ...
        if (boost_flag_CookerView_Up_Right_Down_Right_Down_Left && cookerViewUpRight.getTimeForGearB().equals(TIMER_ZERO_STRING)) {
            ...
            if (workMode == WORK_MODE_HOB && currentCooker == COOKER_ID_Up_Right_Down_Right_Down_Left) {// 右上、右上、左下
                updateCircleProgressValue(value);
            }
        }

        if (boost_flag_CookerView_Up_Left_Down_Right_Middle && cookerViewMiddle.getTimeForGearB().equals(TIMER_ZERO_STRING)) {
            cookerViewUpLeft.SetBoostWorkingStatus(false);
            cookerViewDownRight.SetBoostWorkingStatus(false);
            cookerViewMiddle.SetBoostWorkingStatus(false);
        ...
        if (boost_flag_CookerView_Up_Right_Down_Left_Middle && cookerViewMiddle.getTimeForGearB().equals(TIMER_ZERO_STRING)) {
            cookerViewUpRight.SetBoostWorkingStatus(false);
            cookerViewDownLeft.SetBoostWorkingStatus(false);
            cookerViewMiddle.SetBoostWorkingStatus(false);
        ...
        if (boost_flag_CookerView_Up_Left_Up_Right_Middle && cookerViewMiddle.getTimeForGearB().equals(TIMER_ZERO_STRING)) {
            cookerViewUpLeft.SetBoostWorkingStatus(false);
            cookerViewUpRight.SetBoostWorkingStatus(false);
            cookerViewMiddle.SetBoostWorkingStatus(false);
            boost_flag_CookerView_Up_Left_Up_Right_Middle = false;// 左上、中间、右上
            if (workMode == WORK_MODE_HOB && currentCooker == COOKER_ID_Up_Left_Up_Right_Middle) {// 左上、中间、右上
                updateCircleProgressValue(value);
            }
        }

        if (boost_flag_CookerView_Down_Left_Down_Right_Middle && cookerViewMiddle.getTimeForGearB().equals(TIMER_ZERO_STRING)) {
            cookerViewDownLeft.SetBoostWorkingStatus(false);
            cookerViewDownRight.SetBoostWorkingStatus(false);
            cookerViewMiddle.SetBoostWorkingStatus(false);
            boost_flag_CookerView_Down_Left_Down_Right_Middle = false;// 左下、中间、右下
            if (workMode == WORK_MODE_HOB && currentCooker == COOKER_ID_Down_Left_Down_Right_Middle) {// 左下、中间、右下
                updateCircleProgressValue(value);
            }
        }

        if (boost_flag_CookerView_Up_Left_Down_Left_Middle && cookerViewMiddle.getTimeForGearB().equals(TIMER_ZERO_STRING)) {
            cookerViewUpLeft.SetBoostWorkingStatus(false);
            cookerViewDownLeft.SetBoostWorkingStatus(false);
            cookerViewMiddle.SetBoostWorkingStatus(false);
            boost_flag_CookerView_Up_Left_Down_Left_Middle = false;// 左上、左下、中间
            if (workMode == WORK_MODE_HOB && currentCooker == COOKER_ID_Up_Left_Down_Left_Middle) {// 左上、左下、中间
                updateCircleProgressValue(value);
            }
        }

        if (boost_flag_CookerView_Up_Left_Midlle_Down_Left && cookerViewMiddle.getTimeForGearB().equals(TIMER_ZERO_STRING)) {
            cookerViewUpLeft.SetBoostWorkingStatus(false);
            cookerViewDownLeft.SetBoostWorkingStatus(false);
            cookerViewMiddle.SetBoostWorkingStatus(false);
            boost_flag_CookerView_Up_Left_Midlle_Down_Left = false;//左上、中间、左下
            if (workMode == WORK_MODE_HOB && currentCooker == COOKER_ID_Up_Left_Midlle_Down_Left) {//左上、中间、左下
                updateCircleProgressValue(value);
            }
        }

        if (boost_flag_CookerView_Down_Right_Middle_Up_Right && cookerViewMiddle.getTimeForGearB().equals(TIMER_ZERO_STRING)) {
            cookerViewDownRight.SetBoostWorkingStatus(false);
            cookerViewUpRight.SetBoostWorkingStatus(false);
            cookerViewMiddle.SetBoostWorkingStatus(false);
            boost_flag_CookerView_Down_Right_Middle_Up_Right = false;//右上、中间、右下
            if (workMode == WORK_MODE_HOB && currentCooker == COOKER_ID_Down_Right_Middle_Up_Right) {//右上、中间、右下
                updateCircleProgressValue(value);
            }
        }

        if (boost_flag_CookerView_Down_Right_Up_Right_Middle && cookerViewMiddle.getTimeForGearB().equals(TIMER_ZERO_STRING)) {
            cookerViewDownRight.SetBoostWorkingStatus(false);
            cookerViewUpRight.SetBoostWorkingStatus(false);
            cookerViewMiddle.SetBoostWorkingStatus(false);
            boost_flag_CookerView_Down_Right_Up_Right_Middle = false;// 右上、右下、中间
            if (workMode == WORK_MODE_HOB && currentCooker == COOKER_ID_Down_Right_Up_Right_Middle) {//右上、右下、中间
                updateCircleProgressValue(value);
            }
        }

        if (boost_flag_CookerView_Down_Right_Up_Right_Middle_Down_Left && cookerViewMiddle.getTimeForGearB().equals(TIMER_ZERO_STRING)) {
            cookerViewDownRight.SetBoostWorkingStatus(false);
            cookerViewUpRight.SetBoostWorkingStatus(false);
            cookerViewMiddle.SetBoostWorkingStatus(false);
            cookerViewDownLeft.SetBoostWorkingStatus(false);
        ...
        if (boost_flag_CookerView_Down_Right_Up_Right_Middle_Up_Left && cookerViewMiddle.getTimeForGearB().equals(TIMER_ZERO_STRING)) {
            cookerViewDownRight.SetBoostWorkingStatus(false);
            cookerViewUpRight.SetBoostWorkingStatus(false);
            cookerViewMiddle.SetBoostWorkingStatus(false);
            cookerViewUpLeft.SetBoostWorkingStatus(false);
        ...
        if (boost_flag_CookerView_Down_Left_Up_Left_Middle_Up_Right && cookerViewMiddle.getTimeForGearB().equals(TIMER_ZERO_STRING)) {
            cookerViewDownLeft.SetBoostWorkingStatus(false);
            cookerViewUpLeft.SetBoostWorkingStatus(false);
            cookerViewMiddle.SetBoostWorkingStatus(false);
            cookerViewUpRight.SetBoostWorkingStatus(false);
        ...
        if (boost_flag_CookerView_Down_Left_Up_Left_Middle_Down_Right && cookerViewMiddle.getTimeForGearB().equals(TIMER_ZERO_STRING)) {
            cookerViewDownLeft.SetBoostWorkingStatus(false);
            cookerViewUpLeft.SetBoostWorkingStatus(false);
            cookerViewMiddle.SetBoostWorkingStatus(false);
            cookerViewDownRight.SetBoostWorkingStatus(false);
        ...
        if (boost_flag_CookerView_Up_Left_Middle_Down_Right_Down_Left && cookerViewMiddle.getTimeForGearB().equals(TIMER_ZERO_STRING)) {
            cookerViewUpLeft.SetBoostWorkingStatus(false);
            cookerViewMiddle.SetBoostWorkingStatus(false);
            cookerViewDownRight.SetBoostWorkingStatus(false);
            cookerViewDownLeft.SetBoostWorkingStatus(false);
        ...
        if (boost_flag_CookerView_Up_Right_Middle_Down_Left_Down_Right && cookerViewMiddle.getTimeForGearB().equals(TIMER_ZERO_STRING)) {
            cookerViewUpRight.SetBoostWorkingStatus(false);
            cookerViewMiddle.SetBoostWorkingStatus(false);
            cookerViewDownRight.SetBoostWorkingStatus(false);
            cookerViewDownLeft.SetBoostWorkingStatus(false);
        ...
        if (boost_flag_CookerView_Down_Left_Middle_Up_Right_Up_Left && cookerViewMiddle.getTimeForGearB().equals(TIMER_ZERO_STRING)) {
            cookerViewDownLeft.SetBoostWorkingStatus(false);
            cookerViewMiddle.SetBoostWorkingStatus(false);
            cookerViewUpRight.SetBoostWorkingStatus(false);
            cookerViewUpLeft.SetBoostWorkingStatus(false);
        ...
        if (boost_flag_CookerView_Down_Right_Middle_Up_Left_Up_Right && cookerViewMiddle.getTimeForGearB().equals(TIMER_ZERO_STRING)) {
            cookerViewDownRight.SetBoostWorkingStatus(false);
            cookerViewMiddle.SetBoostWorkingStatus(false);
            cookerViewUpRight.SetBoostWorkingStatus(false);
            cookerViewUpLeft.SetBoostWorkingStatus(false);
        ...
        if (boost_flag_CookerView_All && cookerViewMiddle.getTimeForGearB().equals(TIMER_ZERO_STRING)) {
            cookerViewDownLeft.SetBoostWorkingStatus(false);
            cookerViewUpLeft.SetBoostWorkingStatus(false);
            cookerViewMiddle.SetBoostWorkingStatus(false);
            cookerViewUpRight.SetBoostWorkingStatus(false);
        ...
        if (boost_flag_CookerView_ALL_ANY && cookerViewMiddle.getTimeForGearB().equals(TIMER_ZERO_STRING)) {
            cookerViewDownLeft.SetBoostWorkingStatus(false);
            cookerViewUpLeft.SetBoostWorkingStatus(false);
            cookerViewMiddle.SetBoostWorkingStatus(false);
            cookerViewUpRight.SetBoostWorkingStatus(false);
            ...
            if (workMode == WORK_MODE_HOB && currentCooker == COOKER_ID_ALL_ALL_ANY) { //全部 有区
                updateCircleProgressValue(value);
            }
        }

        if (boost_flag_CookerView_ALL_ALL_LEFT_NONE && cookerViewMiddle.getTimeForGearB().equals(TIMER_ZERO_STRING)) {
            cookerViewDownLeft.SetBoostWorkingStatus(false);
            cookerViewUpLeft.SetBoostWorkingStatus(false);
            cookerViewMiddle.SetBoostWorkingStatus(false);
            cookerViewUpRight.SetBoostWorkingStatus(false);
        ...
        if (boost_flag_CookerView_ALL_ALL_NONE_RIGHT && cookerViewMiddle.getTimeForGearB().equals(TIMER_ZERO_STRING)) {
            cookerViewDownLeft.SetBoostWorkingStatus(false);
            cookerViewUpLeft.SetBoostWorkingStatus(false);
            cookerViewMiddle.SetBoostWorkingStatus(false);
            cookerViewUpRight.SetBoostWorkingStatus(false);
    ...
    public void onViewClicked(View view) {
        switch (view.getId()) {
            case R.id.tv_pause:
                ...
                handler.sendEmptyMessageDelayed(HANDLER_PAUSE_SHOW_TEXT, 1000);
            ...
            case R.id.tv_timer_cancel:   // 恢复上一个界面
                ...
                ReadyToSetTimer = false;
                break;
            case R.id.tv_timer_hour:   // 定时界面 ，小时
                workMode = WORK_MODE_SET_TIMER_HOUR;
                switchTimerSetMode(workMode);
                break;
            case R.id.tv_timer_minute:  // 定时界面，分钟
    ...
    private boolean currentCookerPoweredOn() {
        ...
        switch (currentCooker) {
            ...
            case COOKER_ID_Middle:
                if (cookerViewMiddle.getGearValue() > 0) {
                    flag = true;
                }
                break;
            ...
            case COOKER_ID_Up_Left__Middle:// 左上、中间
                if (cookerViewUpLeft.getGearValue() > 0
                        || cookerViewMiddle.getGearValue() > 0) {
                    flag = true;
                }
                break;
    ...
    private void switchTimerSetMode(int mode) {
        ...
        if (timerHour > maxHour) {
            ...
            if (maxMinutes != 59) {
                timerMinute = maxMinutes;
                tvTimerMinute.setText(String.format(MINUTE_FORMAT, timerMinute));
                LogUtil.d("get the timerMinute is "+timerMinute);
            }
        }
        if (mode == WORK_MODE_SET_TIMER_HOUR) {
            ...
            LogUtil.d("The timerMinute 1 is "+timerMinute);
            if (timerHour == 0) {
            ...
            LogUtil.d("The timerMinute 2 is "+timerMinute);
        }
    }
  ...
  /*      upLeftCookerFramelayout.setScaleX(0.98f);
    ...
    private void doHoodStop() {
        ...
        if (workMode == WORK_MODE_SET_LIGHT) {
            ...
            if (!setPaddingFlagForFanScreen) {
                setPaddingFlagForFanScreen = true;
                tvValueHint.setPadding(0, 0, everyForFanScreen, 0);
            }
    ...
    private void doSetBoostMode_Linked_Cooker() {
        switch (currentCooker) {
            ...
            case COOKER_ID_Down_Left_Down_Right_Middle:// 左下、中间、右下
                ...
                break;
            case COOKER_ID_Up_Left_Down_Left_Middle:// 左上、左下、中间
                boost_flag_CookerView_Up_Left_Down_Left_Middle = true;// 左上、左下、中间
                cookerViewUpLeft.SetBoostWorkingStatus(true);
                cookerViewDownLeft.SetBoostWorkingStatus(true);
                cookerViewMiddle.SetBoostWorkingStatus(true);
                break;
            ...
            case COOKER_ID_Down_Right_Middle_Up_Right://右上、中间、右下
                ...
                break;
            case COOKER_ID_Down_Right_Up_Right_Middle:// 右上、右下、中间
                boost_flag_CookerView_Down_Right_Up_Right_Middle = true;// 右上、右下、中间
                cookerViewDownRight.SetBoostWorkingStatus(true);
                cookerViewUpRight.SetBoostWorkingStatus(true);
                cookerViewMiddle.SetBoostWorkingStatus(true);
                break;
    ...
    private void SetAllCookerIsGray() {
        ...
        cookerViewUpLeft.SetBackGroundGray();
        cookerViewDownLeft.SetBackGroundGray();
        cookerViewMiddle.SetBackGroundGray();
        cookerViewUpRight.SetBackGroundGray();
        cookerViewDownRight.SetBackGroundGray();
        ...
        if (workMode == WORK_MODE_HOB || workMode == WORK_MODE_SET_TIMER_MINUTE || workMode == WORK_MODE_SET_TIMER_HOUR) {  // 处在 定时设置状态也可以选择炉头，2018年11月23日16:09:46
            ...
            switch (id) {
                ...
                case COOKER_ID_Down_Left:
                    ...
                    break;
                case COOKER_ID_Middle:
                    cookerViewMiddle.setGear(TFTCookerConstant.GEAR_DEFAULT_VALUE);
                    cookerViewMiddle.SetBackGroundBlue();
                    break;
                case COOKER_ID_Up_Right:
    ...
    private void ResetValueOfFlag() {
        ...
        if (cookerViewMiddle.getGearValue() == 0) {
            Select_CookerView_ALL_ALL_ANY = false;//全部 有区
            Select_CookerView_Up_Left__Middle = false;// 左上、中间 同时
            Select_CookerView_Down_Left_Middle = false;// 左下、中间 同时
            Select_CookerView_Up_Right_Middle = false;// 右上、中间 同时
            Select_CookerView_Down_Right_Middle = false;// 右下、中间 同时
    ...
    private void doPowerOffCooker(int cooker, int powerOffType) {
        ...
        try {
            ...
            if (activeCookers.size() > 0) {  // 还有炉头在工作
                if (cookerSelected) {
                    int cookerId = activeCookers.pop();
                    int cookerValue = getCookerGearValue(cookerId);
                    doSelectCooker(cookerId, cookerValue);
                }
            } else {    // 没有炉头工作了
                if (TFTCookerApplication.getInstance().isHoodAuto()) {
    ...
    private void doPowerOnTheMiddleCooker(int ValueGear) {  // 中间 炉头 开启或选择
        cookerViewMiddle.powerOn();
        cookerViewMiddle.SetBackGroundBlue();
        cookerViewMiddle.setGear(ValueGear);
    ...
    private int getMaxHobLevel(int cookerId, int cookerGear) {
        ...
        cookerDataEx.setcMode(1);
        cookerDataEx.seteMode(rightCookersUnited() ? 5 : 1);
        cookerDataEx.setfMode(rightCookersUnited() ? 5 : 1);
        cookerDataEx.setaFireValue(cookerViewDownLeft.getGearValue());
        cookerDataEx.setbFireValue(cookerViewUpLeft.getGearValue());
        cookerDataEx.setcFireValue(cookerViewMiddle.getGearValue());
        cookerDataEx.seteFireValue(cookerViewUpRight.getGearValue());
        cookerDataEx.setfFireValue(cookerViewDownRight.getGearValue());
        ...
        switch (cookerId) {
            ...
            case COOKER_ID_Up_Right:
                return cookerPowerDataEx.checkCookerPower(cookerGear, cdE);
            case COOKER_ID_Down_Right:
                return cookerPowerDataEx.checkCookerPower(cookerGear, cdF);
            case COOKER_ID_Up_Left_And_Down_Letf_SameTime: // 左上，左下 同时
            ...
            case COOKER_ID_Up_Right_And_Down_Right_SameTime:   // 右上、右下 同时点击
                return cookerPowerDataEx.checkCookerPower(cookerGear, cdE, cdF);
            case COOKER_ID_Up_Right_Down_Right:   // 右上、右下  滑动
                return cookerPowerDataEx.checkCookerPower(cookerGear, cdEFree, cdFFree);
            case COOKER_ID_Up_Left_Down_Right: // 左上、右下  同时
                return cookerPowerDataEx.checkCookerPower(cookerGear, cdB, cdF);
            case COOKER_ID_Up_Right_Down_Left: // 右上、左下  同时
                return cookerPowerDataEx.checkCookerPower(cookerGear, cdA, cdE);
            case COOKER_ID_Up_Right_Up_Left:// 右上、左上  同时
                return cookerPowerDataEx.checkCookerPower(cookerGear, cdB, cdE);
            case COOKER_ID_Up_Right_And_Up_Left_Slip:  // 右上、左上  滑动
                return cookerPowerDataEx.checkCookerPower(cookerGear, cdB, cdE);
            case COOKER_ID_Down_Right_And_Down_Left_Slip: // 右下、左下  滑动
                return cookerPowerDataEx.checkCookerPower(cookerGear, cdA, cdF);
            case COOKER_ID_Down_Right_Down_Left:// 左下、右下  同时
                return cookerPowerDataEx.checkCookerPower(cookerGear, cdA, cdF);
            ...
            case COOKER_ID_Up_Right_Middle:// 右上、中间  同时点击
                return cookerPowerDataEx.checkCookerPower(cookerGear, cdC, cdE);
            case COOKER_ID_Up_Right_Middle_Slip:// 右上、中间  滑动
                return cookerPowerDataEx.checkCookerPower(cookerGear, cdC, cdE);
            case COOKER_ID_Down_Right_Middle:// 右下、中间 同时点击
                return cookerPowerDataEx.checkCookerPower(cookerGear, cdC, cdF);
            case COOKER_ID_Down_Right_Middle_Slip:// 右下、中间  滑动
                return cookerPowerDataEx.checkCookerPower(cookerGear, cdC, cdF);
            case COOKER_ID_Up_Right_Up_Left_Down_Left: // 右上、左上、左下
                return cookerPowerDataEx.checkCookerPower(cookerGear, cdAFree, cdBFree, cdE);
            case COOKER_ID_Up_Left_Up_Right_Down_Right: // 右上、右下、左上
                return cookerPowerDataEx.checkCookerPower(cookerGear, cdB, cdEFree, cdFFree);
            case COOKER_ID_Up_Right_Down_Right_Down_Left: // 右上、右下、左下
                return cookerPowerDataEx.checkCookerPower(cookerGear, cdA, cdEFree, cdFFree);
            case COOKER_ID_Up_Left_Down_Left_Down_Right: // 左上、左下、右下
                return cookerPowerDataEx.checkCookerPower(cookerGear, cdAFree, cdBFree, cdF);
            case COOKER_ID_Up_Left_Down_Left_Down_Right_Up_Right:  //左上、左下、右下、右上
                return cookerPowerDataEx.checkCookerPower(cookerGear, cdAFree, cdBFree, cdEFree, cdFFree);
            case COOKER_ID_Up_Left_Up_Right_Down_Right_Down_Left://左上、右上、右下、左下
                return cookerPowerDataEx.checkCookerPower(cookerGear, cdA, cdB, cdEFree, cdFFree);
            case COOKER_ID_Up_Right_Up_Left_Down_Left_Down_Right://右上、左上、左下、右下
                return cookerPowerDataEx.checkCookerPower(cookerGear, cdAFree, cdBFree, cdE, cdF);
            case COOKER_ID_Up_Left_Down_Right_Middle:// 左上、中间、右下
                return cookerPowerDataEx.checkCookerPower(cookerGear, cdB, cdC, cdF);
            case COOKER_ID_Up_Right_Down_Left_Middle:// 右上、中间、左下
                return cookerPowerDataEx.checkCookerPower(cookerGear, cdA, cdC, cdE);
            case COOKER_ID_Up_Left_Up_Right_Middle:// 左上、中间、右上
                return cookerPowerDataEx.checkCookerPower(cookerGear, cdB, cdC, cdE);
            case COOKER_ID_Down_Left_Down_Right_Middle:// 左下、中间、右下
                return cookerPowerDataEx.checkCookerPower(cookerGear, cdA, cdC, cdF);
            case COOKER_ID_Up_Left_Down_Left_Middle:// 左上、左下、中间
                return cookerPowerDataEx.checkCookerPower(cookerGear, cdAFree, cdBFree, cdC);
            case COOKER_ID_Down_Right_Up_Right_Middle:// 右上、右下、中间
                return cookerPowerDataEx.checkCookerPower(cookerGear, cdC, cdEFree, cdFFree);
            case COOKER_ID_Up_Left_Midlle_Down_Left://左上、中间、左下
                return cookerPowerDataEx.checkCookerPower(cookerGear, cdA, cdB, cdC);
            case COOKER_ID_Down_Right_Middle_Up_Right://右上、中间、右下:
                return cookerPowerDataEx.checkCookerPower(cookerGear, cdC, cdE, cdF);
            case COOKER_ID_Down_Right_Up_Right_Middle_Down_Left:// 右下、右上、中间、左下
                return cookerPowerDataEx.checkCookerPower(cookerGear, cdA, cdC, cdEFree, cdFFree);
            case COOKER_ID_Down_Right_Up_Right_Middle_Up_Left:// 右下、右上、中间、左上
                return cookerPowerDataEx.checkCookerPower(cookerGear, cdB, cdC, cdEFree, cdFFree);
            case COOKER_ID_Down_Left_Up_Left_Middle_Up_Right:// 左下、左上、中间、右上
                return cookerPowerDataEx.checkCookerPower(cookerGear, cdAFree, cdBFree, cdC, cdE);
            case COOKER_ID_Down_Left_Up_Left_Middle_Down_Right: // 左下、左上、中间、右下
                return cookerPowerDataEx.checkCookerPower(cookerGear, cdAFree, cdBFree, cdC, cdF);
            case COOKER_ID_Up_Left_Middle_Down_Right_Down_Left:// 左上、中间、右下、左下
                return cookerPowerDataEx.checkCookerPower(cookerGear, cdA, cdB, cdC, cdF);
            case COOKER_ID_Up_Right_Middle_Down_Left_Down_Right:// 右上、中间、左下、右下
                return cookerPowerDataEx.checkCookerPower(cookerGear, cdA, cdC, cdE, cdF);
            case COOKER_ID_Down_Left_Middle_Up_Right_Up_Left: // 左下、中间、右上、左上
                return cookerPowerDataEx.checkCookerPower(cookerGear, cdA, cdB, cdC, cdE);
            case COOKER_ID_Down_Right_Middle_Up_Left_Up_Right: // 右下、中间、左上、右上
                return cookerPowerDataEx.checkCookerPower(cookerGear, cdB, cdC, cdE, cdF);
            case COOKER_ID_All: //全部
                return cookerPowerDataEx.checkCookerPower(cookerGear, cdAFree, cdBFree, cdC, cdEFree, cdFFree);
            case COOKER_ID_ALL_ALL_LEFT_NONE: //全部 左边无区
                return cookerPowerDataEx.checkCookerPower(cookerGear, cdAFree, cdBFree, cdC, cdE, cdF);
            case COOKER_ID_ALL_ALL_NONE_RIGHT: //全部 右边无区
                return cookerPowerDataEx.checkCookerPower(cookerGear, cdA, cdB, cdC, cdEFree, cdFFree);
            case COOKER_ID_ALL_ALL_ANY:    //全部 有区
                return cookerPowerDataEx.checkCookerPower(cookerGear, cdA, cdB, cdC, cdE, cdF);
        }
    ...
    private int SetTheTimerValueOfCookerWhenLinked() {  // 检测 炉头是 在定时 状态，用于复制
        ...
        switch (TheFirstClickedCooker) {
            ...
            case COOKER_ID_Down_Left:
                ...
                break;
            case COOKER_ID_Middle:
                if (cookerViewMiddle.isSetTimerToDefault()) {  // 当前没有定时操作

                } else {
                    timerHour = cookerViewMiddle.getTimerHour();//getCurrentCookerTimerHour();
                    timerMinute = cookerViewMiddle.getTimerMinute();//getCurrentCookerTimerMinute();
                    IsDoingTimer = cookerViewMiddle.getTimerMode();
                    if (IsDoingTimer) {
                        timerMode = mode_set_to_alart;  // 当前进行的是 闹铃定时  set to alart
                    } else {
                        timerMode = mode_set_to_stop; // 当前进行的是 关闭定时 set to stop
        ...
        switch (TheFirstClickedCooker) {
            case COOKER_ID_Up_Left:
                ...
                if (v == 0) {
                    ...
                    //  switchCircleProgressMode(TFTCookerConstant.GEAR_MAX_VALUE, v, true);
                }
                break;
            case COOKER_ID_Down_Left:
                ...
                break;
            case COOKER_ID_Middle:
                v = cookerViewMiddle.getGearValue();
                if (v == 0) {
                    //   switchCircleProgressMode(TFTCookerConstant.GEAR_MAX_VALUE, TFTCookerConstant.GEAR_DEFAULT_VALUE, true);
                    v = TFTCookerConstant.GEAR_DEFAULT_VALUE;
                } else {
    ...
    private Map<Integer, Integer> getCookerViewsInProcess() {
        ...
        if (cookerViewMiddle.getGearValue() > TFTCookerConstant.GEAR_DEFAULT_VALUE_ZERO) {
            cookers.put(COOKER_ID_Middle, cookerViewMiddle.getGearValue());
        }
        return cookers;
    }
    ...
    private void SaveAllCookersData() {
        DatabaseHelper.SaveCookersData(0L,
                ...
                1, getRefinedGearValue(cookerViewMiddle), 0,
                1, 0, 0,
    ...
    public void onMessageEvent(NoPanDetectedEvent event) {
        ...
        switch (eventType) {
            case NoPanDetectedEvent.EVENT_TYPE_SET_UI:
                ...
                } else if (cookerViewId == cookerViewMiddle.getId()) {
                }
    ...
    private boolean cookerIsActive(int id) {
        return (id == cookerViewUpLeft.getId() && isCookerSelected(COOKER_ID_Up_Left))
                ...
                || (id == cookerViewMiddle.getId() && isCookerSelected(COOKER_ID_Middle))
    ...
    private void CheckRemovingPan(AnalysisSerialData event) {
        if (event.isPoweredOn()) {// 开机
            ...
            boolean hasPanE = (byte) (event.getEInfo() & 0x03) != 0x00;
            boolean hasPanF = (byte) (event.getFInfo() & 0x03) != 0x00;

            noPanLeftCookers = !hasPanA && !hasPanB;
            noPanRightCookers = !hasPanE && !hasPanF;
            if (leftCookersUnited()) {
                if (cookerViewDownLeft.getGearValue() > 0) {
            ...
            if (cookerViewMiddle.getGearValue() > 0) {
                cookerViewMiddle.CheckRemovingPan(hasPanC);
            }

            if (rightCookersUnited()) {
                if (cookerViewUpRight.getGearValue() > 0) {
                    cookerViewUpRight.CheckRemovingPan(hasPanE || hasPanF);
                    cookerViewDownRight.CheckRemovingPan(hasPanE || hasPanF);
                    if (hasPanE || hasPanF) {
                ...
                if (cookerViewUpRight.getGearValue() > 0) {
                    cookerViewUpRight.CheckRemovingPan(hasPanE);
                }
                if (cookerViewDownRight.getGearValue() > 0) {
                    cookerViewDownRight.CheckRemovingPan(hasPanF);
                }
            }
        }
    }
    ...
    private void checkCookerHighTemperature(AnalysisSerialData event) {
        if (event.isPoweredOn()) {
            ...
            boolean highPanE = (byte) (event.getEInfo() & 0x08) == 0x08;
            boolean highPanF = (byte) (event.getFInfo() & 0x08) == 0x08;
            cookerViewDownLeft.ShowHighTempStatus(highPanA);
            cookerViewUpLeft.ShowHighTempStatus(highPanB);
            cookerViewMiddle.ShowHighTempStatus(highPanC);
            cookerViewUpRight.ShowHighTempStatus(highPanE);
            cookerViewDownRight.ShowHighTempStatus(highPanF);
            if (highPanA || highPanB || highPanC || highPanE || highPanF) {
                // 有高温时，不进行待机计时
                TFTCookerApplication.getInstance().updateLatestTouchTime();
    ...
    protected void handleMessage(Message msg) {
        switch (msg.what) {
            ...
            case HANDLER_TOUCH_SCREEN_ONE_MINUTE:  // 检测一分钟是否可以退到屏蔽状态
                if (cookerViewUpLeft.getGearValue() == 0 && cookerViewDownLeft.getGearValue() == 0 && cookerViewUpRight.getGearValue() == 0 &&
                        cookerViewDownRight.getGearValue() == 0 && cookerViewMiddle.getGearValue() == 0) {
    ...
    private void PowerOffWhenNoPan(int value) {
        ...
        } else if (value == cookerViewMiddle.getId()) {  // 中间
            cookerViewMiddle.powerOff();
