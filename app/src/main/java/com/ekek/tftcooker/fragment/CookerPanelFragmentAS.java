...
public class CookerPanelFragmentAS extends CookerPanelFragment
    ...
    TextView tvTimer;
    @BindView(R.id.tv_timer_operate_hood)
    TextView tvTimerHood;
    @BindView(R.id.tv_lock)
    TextView tvLock;
    @BindView(R.id.tv_boost)
    TextView tvBoost;
    @BindView(R.id.tv_boost_hood)
    TextView tvBoostHood;
    ...
    @BindView(R.id.llHoodDisplay)
    LinearLayout llHoodDisplay;
    @BindView(R.id.tvHoodDisplay)
    TextView tvHoodDisplay;
    ...
    private HoodTimerThread hoodTimerThread = null;
    private boolean doingPowerOffCooker = false;

    @Override
    protected int initLayout() {
        return R.layout.fragment_cooker_panel_fragment_a;
    }
    ...
    public void onProgress(float value, boolean fromWidget) {
        ...
        if (workMode == WORK_MODE_HOB) {// 炉灶 模式
            ...
            if (hoodGear == TFTCookerConstant.HOOD_MAX_VALUE && !TFTCookerApplication.getInstance().isHoodAuto()) {
                changeTheBoostHoodButtonColor(ColorOfBoostButton_colorOrange);
            } else {
                changeTheBoostHoodButtonColor(ColorOfBoostButton_colorWhite);
            }
    ...
    @OnClick({
            R.id.tv_pause,
            R.id.tv_timer_operate,
            R.id.tv_timer_operate_B18,
            R.id.tv_timer_operate_hood,
            R.id.tv_lock,
            R.id.tv_lock_m,
            R.id.tv_boost,
            R.id.tv_boost_hood,
            ...
            R.id.tv_auto,
            R.id.tv_hood_stop,
            R.id.tv_timer_set_to_stop,
            R.id.tv_timer_set_to_alert,
            R.id.tv_timer_cancel,
            R.id.tv_timer_hour,
            R.id.tv_timer_minute,
            R.id.llHoodDisplay})
    public void onViewClicked(View view) {
        switch (view.getId()) {
            ...
            case R.id.tv_timer_operate_B18:
                ...
                break;
            case R.id.tv_timer_operate_hood:
                // 风机定时
                doHoodTimer();
                break;
            ...
            case R.id.tv_boost:  // 升温，
                ...
                doSwitchBoostMode();
                break;
            case R.id.tv_boost_hood:
                cancelHoodTimer();
                doSetHoodGear(TFTCookerConstant.HOOD_MAX_VALUE);
                break;
            ...
            case R.id.tv_timer_minute:
                ...
                switchTimerSetMode(workMode);
                break;
            case R.id.llHoodDisplay:
                if (workMode != WORK_MODE_HOB && workMode != WORK_MODE_HOOD) {
                    return;
                }
    ...
    private void switchTimerSetMode(int mode) {
        ...
        if (mode == WORK_MODE_SET_TIMER_HOUR) {
            tvTimerHour.setTextColor(Color.parseColor("#1593d2"));
            ...
            tvTimerMinute.setTextColor(Color.parseColor("#1593d2"));
    ...
    private void switchTimerDisplay(TextView view, boolean pressed) {
        if (pressed) {
    ...
    private void doHoodStop() {
        ...
        if (workMode == WORK_MODE_SET_LIGHT) {
            lightGear_blue = 0;
            switchCircleProgressMode(TFTCookerConstant.HOOD_LIGHT_MAX_VALUE, lightGear_blue, true);
    ...
    private void doSetHoodGear(int gear) {
        hoodGear = gear;
        TFTCookerApplication.getInstance().setHoodAuto(false);
    ...
    private void doHoodTimer() {
        if (hoodGear == 0) return;

        TFTCookerApplication.getInstance().setHoodAuto(false);
        SetHoodSendGear(hoodGear);
        setAutoUI();

        if (hoodTimerThread != null && hoodTimerThread.isAlive()) {
            switchTimerDisplay(tvTimerHood, false);
            cancelHoodTimer();
        } else {
            switchTimerDisplay(tvTimerHood, true);
            hoodTimerThread = new HoodTimerThread();
            hoodTimerThread.start();
        }
    }
    private void cancelHoodTimer() {
        if (hoodTimerThread != null) {
            hoodTimerThread.setCancelTask(true);
            try {
                hoodTimerThread.join(2000);
            } catch (InterruptedException e) {
                Logger.getInstance().e(e);
            }
            hoodTimerThread = null;
        }
    }
    private void pauseHoodTimer() {
        if (hoodTimerThread != null) {
            hoodTimerThread.setPauseTask(true);
        }
    }
    private void resumeHoodTimer() {
        if (hoodTimerThread != null) {
            hoodTimerThread.setPauseTask(false);
        }
    }
    ...
    private void refreshHoodCircle() {
        if (workMode == WORK_MODE_HOOD) {
            ...
            if (TFTCookerApplication.getInstance().isHoodAuto()) {
                tvValue.setText(R.string.title_auto_short);
            } else if (hoodGear == TFTCookerConstant.HOOD_MAX_VALUE) {
    ...
    private void refreshHoodDisplay() {
        if (TFTCookerApplication.getInstance().isHoodAuto()) {
            tvHoodDisplay.setText(R.string.title_auto_short);
        } else if (hoodGear == TFTCookerConstant.HOOD_MAX_VALUE) {
            tvHoodDisplay.setText(R.string.title_boost_short);
        } else {
            tvHoodDisplay.setText(String.valueOf(hoodGear));
        }
    }
    ...
    private void changeTheBoostHoodButtonColor(int v) {
        Drawable dw;
        switch (v) {
            case ColorOfBoostButton_colorOrange:
                tvBoostHood.setTextColor(getResources().getColor(R.color.colorOrange));
                dw = getResources().getDrawable(R.mipmap.boost_press);
                dw.setBounds(0, 0, dw.getMinimumWidth(), dw.getMinimumHeight());
                tvBoostHood.setCompoundDrawables(null, dw, null, null);
                break;
            case ColorOfBoostButton_colorWhite:
                tvBoostHood.setTextColor(getResources().getColor(R.color.colorWhite));
                dw = getResources().getDrawable(R.mipmap.boost_normal);
                dw.setBounds(0, 0, dw.getMinimumWidth(), dw.getMinimumHeight());
                tvBoostHood.setCompoundDrawables(null, dw, null, null);
                break;
    ...
    private void switchWorkMode(int mode) {
        switch (mode) {
            case WORK_MODE_HOB:
                ...
                llHoodDisplay.setBackground(getResources().getDrawable(R.mipmap.hood_display_as));
                tvHoodDisplay.setVisibility(View.VISIBLE);
                showHobOperateMenu();
                hideHoodOperateMenu();
            ...
            case WORK_MODE_HOOD:
                ...
                llHoodDisplay.setBackground(getResources().getDrawable(R.mipmap.hood_display_as_selected));
                tvHoodDisplay.setVisibility(View.VISIBLE);
                showHoodOperateMenu();
                hideHobOperateMenu();
                SetAllCookerIsGray();
    ...
    private void LockAllUI(boolean flag) {
        if (flag) {
            ...
            llHoodDisplay.setEnabled(false);
            circleProgress.disable();
        }
    }
    ...
    private void CheckRemovingPan(AnalysisSerialData event) {
        if (event.isPoweredOn()) {// 开机
            boolean hasPanA = (byte) (event.getAInfo() & 0x01) == 0x01;
            boolean hasPanB = (byte) (event.getBInfo() & 0x01) == 0x01;
            boolean hasPanC = (byte) (event.getCInfo() & 0x01) == 0x01;
            boolean hasPanD = (byte) (event.getDInfo() & 0x01) == 0x01;
    ...
    protected void handleMessage(Message msg) {
        switch (msg.what) {
            ...
            case HANDLER_ADJUST_LEVEL:
                ...
                break;
            case HANDLER_HOOD_GEAR_CHANGED:
                doSetHoodGear(hoodGear);
                break;
            case HANDLER_HOOD_GEAR_BLINK:
                boolean visible = msg.arg1 == 1 ? true : false;
                tvHoodDisplay.setVisibility(visible ? View.VISIBLE: View.INVISIBLE);
                if (workMode == WORK_MODE_HOOD) {
                    tvValue.setVisibility(visible ? View.VISIBLE : View.INVISIBLE);
                } else if (workMode == WORK_MODE_HOB && tvCookwareNotDetected.getVisibility() != View.VISIBLE) {
                    tvValue.setVisibility(View.VISIBLE);
                }
                break;
            case HANDLER_HOOD_TIMER_FINISHED:
                tvHoodDisplay.setVisibility(View.VISIBLE);
                tvValue.setVisibility(View.VISIBLE);
                switchTimerDisplay(tvTimerHood, false);
                break;
            case HANDLER_HOOD_NUMBER_BLINK:
                if (activeCookers.size() > 0 || !TFTCookerApplication.getInstance().isHoodAuto()) {
                    ...
                    // 在烟机界面进行闪烁
                    int visibility = tvValue.getVisibility() == View.VISIBLE ? View.INVISIBLE : View.VISIBLE;
                    tvHoodDisplay.setVisibility(visibility);
    ...
    private class HoodTimerThread extends Thread {
        ...
        public void run() {
            try {
                ...
                int duration;
                long durationBeforePaused = -1L;
                int blink = 0;
                boolean firstTime = true;
                while (!cancelTask && hoodGear > 0) {

                    if (pauseTask) {
                        if (durationBeforePaused == -1L) {
                            durationBeforePaused = SystemClock.elapsedRealtime() - start;
                        }
                        start = SystemClock.elapsedRealtime() - durationBeforePaused;

                        Message msg = new Message();
                        msg.what = HANDLER_HOOD_GEAR_BLINK;
                        msg.arg1 = 1;
                        ...
                        durationBeforePaused = -1L;
                    }

                    // 第一次降档目标为5分钟，其他为2分钟
                    duration = firstTime ? DURATION_FIVE_MINUTES : DURATION_TWO_MINUTES;
                    if (SystemClock.elapsedRealtime() - start >= duration) {
                        hoodGear--;
                        handler.sendEmptyMessage(HANDLER_HOOD_GEAR_CHANGED);
                ...
                handler.sendEmptyMessage(HANDLER_HOOD_TIMER_FINISHED);
